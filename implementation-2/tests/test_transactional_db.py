...
class TestKVStore(unittest.TestCase):
    ...
    def test_concurrent_transaction_isolation(self):
        ...
        def worker1():
            ...
            time.sleep(2)  # Hold the transaction to force worker2 to wait
        ...
        def worker2():
            ...
            # In this implementation(implementation-2), since we are not locking the store on a new transaction, this will result in
            # self.kv.get("shared_key") == "initial" as True, which is corrupted data as worker1 will commit its changes before worker2.
            time.sleep(5)
            self.kv.commit()
